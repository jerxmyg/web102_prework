...
function addGamesToPage(games) {
    ...
    for (let index = 0; index < games.length; index++) {
        ...
        div.innerHTML = `<img class="game-img" src=${game.img} alt=${game.name} />
            <p><b>${game.name}</b></p>
...
raisedCard.innerHTML = `<p>$${amtRaised.toLocaleString('en-US')}</p>`;
...
const unfundedStr = `A total of <b>$${totalContributions.toLocaleString('en-US')}</b> has been raised for <b>${totalGames}</b> games. 
Currently, <b>${unfundedCount} ${unfundedCount < 2 ? "game remains": "games remain"} unfunded</b>. We need
...
/************************************************************************************
 * Extra Customizations: changed the button functionaity to turn black when clicked.
 * reverts to default color when other buttons are clicked.
 * 
 */
